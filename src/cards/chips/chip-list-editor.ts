 ...
 *
 * A chip is an entity plus four things nobody can work out from it — how much of it to draw,
 * which glyph, what to call it, and what a press does — and this draws each row as an
 * `ha-expansion-panel` carrying those fields, with a drag handle beside it and a delete button
 * in its summary. A picker at the bottom adds the next one.
 *
...
const chipSchema = (
  ...
  if (data.action === 'navigate') {
    rows.push({ name: 'navigation_path', selector: { text: { placeholder: '/lovelace/0' } } })
  }
...
const ADD_HELPER =
  ...
  'One already in the list is not offered again.'
...
class CupertinoChipsList extends LitElement {
   ...
   *
   * A row whose entity has been cleared is dropped rather than kept as a chip pointing at
   * nothing — `chipFromForm` answers `undefined` for exactly that, and `chipRows` would drop
   * it anyway. Doing it here as well keeps `_open` from holding a key nothing will render.
   */
  private readonly _rowChanged = (
    ...
    if (row === undefined) {
      ...
      next[index] = row
      // The panel stays open across the rename that a changed entity amounts to. Only for a
      // row whose key is its bare id: a `#1` key is positional (see `chipKeys`) and the new
      // one is not this row's to guess.
      if (key === prior.entity && prior.entity !== row.entity && this._open.delete(key)) {
  ...
  private _renderChip(config: ChipConfig, index: number, key: string): TemplateResult {
    ...
    return html`
      <div class="chip">
        ...
        >
          <ha-icon slot="leading-icon" .icon=${chip.icon}></ha-icon>
          <ha-icon-button
            slot="icons"
            class="remove"
  ...
  protected override render(): TemplateResult | typeof nothing {
    ...
    const taken = this.chips.map(chip => chip.entity)
